...
export const messageModificationPlugin = {
    ...
    hooks: {
         ...
         */
        onRenderMessageControls: function(container, message, chatlog, app) {
            if (!app) return; // Don't render controls if app is not available
            const store = app.store;
            const messageInput = app.ui.messageEl;
            ...
            const addBtn = createControlButton(
                ...
                () => {
                    ...
                    if (message.value.role === 'assistant') {
                        ...
                        const newAlternative = chatlog.addAlternative(message, { role: message.value.role, content: null });
                        UIManager.renderEntireChat(); // Re-render to show the new alternative
                        messageInput.value = originalContent ? originalContent.trim() : '';
                        messageInput.dispatchEvent(new Event('input', { bubbles: true }));
                        store.set('editingPos', chatlog.getMessagePos(newAlternative));
            ...
            const editBtn = createControlButton(
                ...
                () => {
                    ...
                    UIManager.toggleMessageEditMode(message, true);
                    messageInput.focus();
                }
            );
